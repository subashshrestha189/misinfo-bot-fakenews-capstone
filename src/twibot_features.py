...
NUM_BASE = [
    "followers_count","following_count","tweet_count","listed_count","account_age_days"
]
BIN_BASE = ["has_profile_image","has_description","verified","has_location","has_url"]
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["tweets_per_day"] = _safe_ratio(df["tweet_count"], np.where(df["account_age_days"]<=0, 1, df["account_age_days"]))

    # Log transforms (stabilize heavy tails)
    for c in ["followers_count","following_count","tweet_count","listed_count","account_age_days"]:
    ...
    for c in ["followers_following_ratio","tweets_per_day","ff_log_ratio",
              "listed_per_1k_followers","tweets_per_1k_followers"]:
        df[c] = df[c].replace([np.inf, -np.inf], np.nan).fillna(0).clip(0, 1e6)

    # Final feature list (numeric + binaries)
    feat_cols = (
        NUM_BASE
        + ["followers_following_ratio","tweets_per_day","ff_log_ratio",
           "listed_per_1k_followers","tweets_per_1k_followers"]
        + [f"log1p_{c}" for c in ["followers_count","following_count","tweet_count","listed_count","account_age_days"]]
        + BIN_BASE
    )
